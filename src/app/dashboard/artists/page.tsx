...
export default function ArtistsPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        // Fetch top artists
        const artistsData = await fetchWithToken(`/me/top/artists?limit=9&time_range=${timeRange}`)
        
        // Fetch top tracks to calculate artist affinity
        const tracksData = await fetchWithToken(`/me/top/tracks?limit=50&time_range=${timeRange}`)
        
        if (mounted) {
          // Calculate how many times each artist appears in top tracks
